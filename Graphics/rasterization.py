...
def rasterize_points(
    ...
    x, y = torch.meshgrid(
        torch.arange(0, texture_size[1], device=device),
        torch.arange(0, texture_size[0], device=device),
        indexing="ij",
    )
    ...
    )  # / (texture_size * texture_size).sum().sqrt()
    point_distances = torch.exp(-torch.pow(point_distances / sigma, 2))

    return point_distances


def rasterize_points_in_non_ndc(
    points: torch.tensor,
    sigma: float,
    texture_size: torch.tensor,
    device: torch.cuda.device = torch.device("cuda"),
...
def rasterize_depth(
    ...
    point_distances = (
        point_distances
        / point_distances.max(dim=2, keepdim=True)[0].max(dim=1, keepdim=True)[0]
    )
...
def baked_sum(
    ...
    for i in range(points.shape[0]):
        ...
        y, x = torch.meshgrid(
            torch.arange(0, footprint, device=device),
            torch.arange(0, footprint, device=device),
            indexing="ij",
        )
        ...
        rect_end = torch.tensor(
            [footprint, footprint], dtype=torch.int32, device=device
        )
        ...
        tex[wo[0] : wo[0] + re[0] - rs[0], wo[1] : wo[1] + re[1] - rs[1]] = (
            tex[wo[0] : wo[0] + re[0] - rs[0], wo[1] : wo[1] + re[1] - rs[1]].clone()
            + dist[rs[0] : re[0], rs[1] : re[1]]
        )
...
def baked_sum_2(
    ...
    rect_end = (
        torch.zeros(points.shape[0], 2, dtype=torch.int32, device=device) + footprint
    )
    ...
    if (wo[:, 0] + footprint >= texture_size[0]).any():
        rect_end[:, 0] = torch.where(
            wo[:, 0] + footprint >= texture_size[0],
            texture_size[0] - wo[:, 0],
            rect_end[:, 0],
        )

    if (wo[:, 1] + footprint >= texture_size[1]).any():
        rect_end[:, 1] = torch.where(
            wo[:, 1] + footprint >= texture_size[1],
            texture_size[1] - wo[:, 1],
            rect_end[:, 1],
        )
    ...
    for i in range(points.shape[0]):
        tex[
            wo[i, 0] : wo[i, 0] + re[i, 0] - rs[i, 0],
            wo[i, 1] : wo[i, 1] + re[i, 1] - rs[i, 1],
        ] = (
            tex[
                wo[i, 0] : wo[i, 0] + re[i, 0] - rs[i, 0],
                wo[i, 1] : wo[i, 1] + re[i, 1] - rs[i, 1],
            ].clone()
            + dist[i, rs[i, 0] : re[i, 0], rs[i, 1] : re[i, 1]]
        )
...
def baked_softor(
    ...
    for i in range(points.shape[0]):
        ...
        tex[wo[0] : wo[0] + re[0] - rs[0], wo[1] : wo[1] + re[1] - rs[1]] = tex[
            wo[0] : wo[0] + re[0] - rs[0], wo[1] : wo[1] + re[1] - rs[1]
        ].clone() * (1 - dist[rs[0] : re[0], rs[1] : re[1]])
...
def baked_softor_2(
    ...
    for i in range(points.shape[0]):
        tex[
            ...
            1 - dist[i, rs[i, 0] : re[i, 0], rs[i, 1] : re[i, 1]]
        )

    return (1 - tex).T


def rasterize_points_baked_softor(
    points: torch.tensor,
    sigma: float,
    texture_size: torch.tensor,
    device: torch.cuda.device = torch.device("cuda"),
...
# We assume points to be in camera space [0, 1]
def rasterize_points_baked_sum(
    points: torch.tensor,
    sigma: float,
    texture_size: torch.tensor,
    device: torch.cuda.device = torch.device("cuda"),
...
def test_point_reg(reduce_overlap: bool = True):
    ...
    optim = torch.optim.Adam([{"params": points, "lr": 0.001}])
    ...
    for i in tqdm(range(opt_steps)):
        ...
        loss = (
            loss_func(softored, summed)
            if reduce_overlap
            else -loss_func(softored, summed)
        )
        print(loss.item())
        loss.backward()
        optim.step()
        ...
        with torch.no_grad():
            ...
            bla = cv2.applyColorMap(
                (softored.detach().cpu().numpy() * 255).astype(np.uint8),
...
def test_line_reg():
    ...
    location_vector = (
        (torch.rand([num_lines, 2], device=device) - 0.5) * 2.0 / 10.0
    )  # Every line should be roughly in the middle of our frame
    ...
    optim = torch.optim.Adam([{"params": location_vector, "lr": 0.005}])
    ...
    for i in tqdm(range(opt_steps)):
        ...
        lines = torch.concat([p0.unsqueeze(-1), p1.unsqueeze(-1)], dim=-1).transpose(
            1, 2
        )
        ...
        with torch.no_grad():
            ...
            if i == 0 or i == opt_steps - 1:
                fig.savefig(
                    "assets/line_reduced_overlap{0}.eps".format(i),
                    facecolor=ax.get_facecolor(),
                    edgecolor="none",
                    bbox_inches="tight",
                    pad_inches=0,
    ...
    imageio.v3.imwrite(
        "assets/line_regularization.mp4", np.stack(images, axis=0), fps=25
    )
...
def test_square_reg():
    ...
    for i in tqdm(range(opt_steps)):
        ...
        rasterized_lines = rasterize_squares(
            square_centroids, square_sizes, sigma, texture_size
        )
        ...
        with torch.no_grad():
            square_centroids[square_centroids > texture_size[0] - 1] = (
                texture_size[0] - 1
            )
            square_centroids[square_centroids < 0.0] = 0.0
            np_lines = cv2.applyColorMap(
                (softored.detach().cpu().numpy() * 255).astype(np.uint8),
                cv2.COLORMAP_VIRIDIS,
            )
...
def time_it():
    ...
    og_sum = timeit.Timer(
        lambda: rasterize_points(points, sigma, texture_size, device).sum(dim=0)
    )
    print(og_sum.timeit(repeats))

    t_baked_sum = timeit.Timer(
        lambda: baked_sum(points, sigma, texture_size, 4, device)
    )
    print(t_baked_sum.timeit(repeats))

    t_baked_sum2 = timeit.Timer(
        lambda: baked_sum_2(points, sigma, texture_size, 4, device)
    )
    print(t_baked_sum2.timeit(repeats))

    og_softor = timeit.Timer(
        lambda: softor(rasterize_points(points, sigma, texture_size, device))
    )
    print(og_softor.timeit(repeats))

    t_baked_softor1 = timeit.Timer(
        lambda: baked_softor(points, sigma, texture_size, 4, device)
    )
    print(t_baked_softor1.timeit(repeats))

    t_baked_softor2 = timeit.Timer(
        lambda: baked_softor_2(points, sigma, texture_size, 4, device)
    )
