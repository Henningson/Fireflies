...
def render_depth(scene, spp=64):
    ...
    depth = torch.tensor(result).reshape(sensor.film().size()[0], sensor.film().size()[1], -1).mean(dim=-1)
...
def main():
    ...
    for iter in range(iteration_count):
        ...
        cv2.imshow("Rendered Image", rendered_img.detach().cpu().numpy()[:, :, [2, 1, 0]])
        cv2.imshow("Thresholded Image", (thresh_img.detach().cpu().numpy() * 255).astype(np.uint8))
        ...
        with torch.no_grad():
            ...
            error_map = utils_np.normalize(np.abs(bitmap_gray - reference_image.detach().cpu().numpy().mean(axis=-1)))
            error_map = cv2.applyColorMap((error_map*255).astype(np.uint8), cv2.COLORMAP_VIRIDIS)
