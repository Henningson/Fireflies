...
class PointNetRegression(nn.Module):
    def __init__(self, feat, num_params: int = 100):
...
class Model(nn.Module):
    def __init__(
        self,
        config={"shape_params": 100, "expression_params": 50, "num_beams": 400},
        ...
        self.feature_extractor = PointNetfeat(
            global_feat=False, feature_transform=feature_transform
        )
        self.shape_regressor = PointNetRegression(
            self.feature_extractor, num_params=config["shape_params"]
        )
        self.expression_regressor = PointNetRegression(
            self.feature_extractor, num_params=config["expression_params"]
        )
        self._pose_params = torch.zeros(1, 6, device=device)

        self.shapeUnificator = nn.Linear(config["num_beams"], 1)
        self.expressionUnificator = nn.Linear(config["num_beams"], 1)

    def forward(self, x):
        per_point_shape_estimate = self.shape_regressor(x)[0]
        per_point_expression_estimate = self.expression_regressor(x)[0]

        shape_estimates = self.shapeUnificator(per_point_shape_estimate)[:, :, 0]
        expression_estimates = self.expressionUnificator(per_point_expression_estimate)[
            :, :, 0
        ]
        ...
        vertices, _ = self._flame(
            ...
            self._pose_params.repeat(shape_estimates.shape[0], 1),
        )
...
if __name__ == "__main__":
    sim_data = Variable(torch.rand(1, 3, 400, device="cuda"))
    """
    trans = STN3d()
    out = trans(sim_data)
    print('stn', out.size())
    ...
    vertices, shape_estimates, expression_estimates = regr(sim_data)
    print("seg", vertices.shape, shape_estimates.shape, expression_estimates.shape)
