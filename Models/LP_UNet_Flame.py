...
class Model(nn.Module):
    def __init__(self, 
        ...
        self.in_channels = in_channels
        self.laserbeam_to_2d = nn.Linear(config['num_beams']*3, self.output_image_size[0]*self.output_image_size[1]*config['in_channels'])
        ...
        self.expression_layer = nn.Linear(self.output_image_size[0]*self.output_image_size[1]*out_channels, config['expression_params'])
        self.shape_layer = nn.Linear(self.output_image_size[0]*self.output_image_size[1]*out_channels, config['shape_params'])
    ...
    def forward(self, x):
        x = self.laserbeam_to_2d(x.flatten(start_dim=1)).reshape(x.shape[0], self.in_channels, self.output_image_size[0], self.output_image_size[1])
        x = self.encoder(x)
        x = self.bottleneck(x)
        x = self.decoder(x)
        x = self.final_conv(x)
