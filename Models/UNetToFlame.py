...
def test():
    config={'in_channels': 1, 'out_channels': 1, 'features': [32, 64, 128], 'output_image_size': [512, 256], 'shape_params': 100, 'expression_params': 100}
    
    rand_input = torch.randn((4, 1, 512, 256))
    expected_output_size = torch.randn((1, 512, 256))

    model = Model(config)
    shape_params, expression_params = model(rand_input)
