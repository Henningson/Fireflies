...
def laser_from_ndc_points(
    ...
    intersection_distances = fireflies.utils.intersections.rayPlane(
...
def initialize_laser(
    ...
    if mode == "RANDOM":
        local_laser_dir = fireflies.projection.Laser.generate_random_rays(
            num_beams=n_beams, intrinsic_matrix=LASER_K, device=device
        )
    elif mode == "POISSON":
        local_laser_dir = fireflies.projection.Laser.generate_blue_noise_rays(
        ...
        local_laser_dir = fireflies.projection.Laser.generate_uniform_rays_by_count(
        ...
        depth_maps = fireflies.graphics.depth.random_depth_maps(
        ...
        variance_map = fireflies.utils.math.normalize(variance_map)
        ...
        normalized_sampling = 1 - fireflies.utils.math.normalize(final_sampling_map)
        normalized_sampling = (
            min_radius + (max_radius - min_radius) * normalized_sampling
        )
        n_points, points = fireflies.sampling.poisson.bridson(
    ...
    return fireflies.projection.Laser(
