...
class GlobalArgumentParser(argparse.ArgumentParser):
    def __init__(
        ...
        self.add_argument(
            "--sigma", type=float, help="Defines the diameter of the laser beams."
        )
        ...
        self.add_argument(
            "--steps_per_anim",
            type=int,
            help="How many frames should be generated per animation step.",
        )

        self.add_argument(
            "--iterations", type=int, help="Number of optimization iterations."
        )
        self.add_argument(
            "--lr_model", type=float, help="Learning rate of the neural network"
        )
        self.add_argument(
            "--lr_laser", type=float, help="Learning rate of the laser rays."
        )
        self.add_argument(
            "--lr_sigma",
            type=float,
        ...
        self.add_argument(
            "--save_images", type=str2bool, nargs="?", help="Should a video be saved?"
        )
        self.add_argument(
            "--visualize", type=str2bool, nargs="?", help="Should a video be saved?"
        )
        self.add_argument("--upscale", type=float, help="Should a video be saved?")

        self.add_argument("--eval", action="store_true")
        self.add_argument("--eval_dir", action="store_true")
        self.add_argument(
