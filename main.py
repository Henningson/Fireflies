...
def main():
    ...
    chosen_points = LaserEstimation.points_from_probability_distribution(
        final_sampling_map, config.n_beams
    )
    ...
    laser_dir = LaserEstimation.laser_from_ndc_points(
        global_scene.sensors()[0],
        laser_origin,
        depth_maps,
        chosen_points,
        device=DEVICE,
    ...
    Laser = laser.Laser(
        firefly_scene.projector, local_laser_dir, fov, near_clip, far_clip
    )
    ...
    scheduler = torch.optim.lr_scheduler.PolynomialLR(
        optim, total_iters=config.iterations, power=0.99
    )

    upsampling = [
        global_scene.sensors()[0].film().size() // 2**i
    ...
    for i in (progress_bar := tqdm(range(config.iterations))):
        ...
        multi_res_depth = subsampled_point_raster(
            filtered_ndc_points, 6, config.sigma, sensor_size
        )
        ...
        with torch.no_grad():
            for j, mult in enumerate(multi_res_depth):
                ...
                if i % 500 == 0 and i > 0:
                    cv2.imwrite(
                        "Res{}.png".format(j),
                        (mult.detach().cpu().numpy().squeeze() * 255).astype(np.uint8),
                    )
        ...
        with torch.no_grad():
            ...
            if config.visualize:
                ...
                gt_depth_map = (
                    dense_depth.unsqueeze(-1).repeat(1, 1, 3).detach().cpu().numpy()
                )
                ...
                texture = (
                    texture_init.unsqueeze(-1).repeat(1, 1, 3).detach().cpu().numpy()
                )
                epipolar_lines = (
                ...
                cv2.imwrite(
                    "ims/{0:05d}.png".format(i), (concat_im * 255).astype(np.uint8)
                )
