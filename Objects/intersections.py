...
if __name__ == "__main__":
    ...
    for _ in range(1000):
        ...
        for i in range(a_coords.shape[0]):
            cv2.circle(
                image,
                (a_coords[i].detach().cpu().numpy() * 512).astype(np.int),
                (a_radius[i, 0].detach().cpu().numpy() * 512).astype(np.int),
                color=(0, 0, 255) if intersections[i, 0] else (0, 255, 0),
            )
            cv2.circle(
                image,
                (b_coords[i].detach().cpu().numpy() * 512).astype(np.int),
                (b_radius[i, 0].detach().cpu().numpy() * 512).astype(np.int),
                color=(0, 0, 255) if intersections[i, 0] else (0, 255, 0),
            )
